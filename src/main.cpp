...
WebServer server(80); // Port par défaut

const char* ssid = "DrawBotLulu"; // Nom du réseau
const char* password = "Drawbot12"; // Mot de passe du réseau
...
float lireCapMagnetique() {
  ...
  float x_cal = (event.magnetic.x - mag_offset_x) * mag_scale_x;
  float y_cal = (event.magnetic.y - mag_offset_y) * mag_scale_y;
...
void startRobot2() {
  if (server.hasArg("distance")){
    ...
    if (distanceCm >= 2 && distanceCm <= 20){
      ...
      server.send(400,"text/plain","Veuillez choisir une distance entre 2 et 20 cm.");
    }
  }
  else{
    arreterMoteurs();
...
void handleRoot() {
  ...
  page += "<body>";
  page += "    <div class='container'>";
  page += "        <h1>Bienvenue sur la plateforme de contrôle du Drawbot</h1>";
  page += "        <button onclick=\"location.href='/sequence1'\">Séquence 1</button>";
  page += "        <button onclick=\"location.href='/sequence2'\">Séquence 2</button>";
  page += "        <button onclick=\"location.href='/sequence3'\">Séquence 3</button>";
  page += "    </div>";
  page += "</body>";
  page += "</html>";
...
void handleNotFound() {
  server.send(404, "text/plain", "Flop"); // Message d'erreur
}
...
void setup() {
  ...
  server.on("/", handleRoot); // Affiche la page d'accueil
  server.on("/sequence1", startRobot1); // Démarre le robot pour Séquence 1
  server.on("/sequence2", demanderDistance); // Démarre le robot pour Séquence 2
  server.on("/sequence3", startRobot3); // Démarre le robot pour Séquence 3
  server.on("/start-stairs",startRobot2);
  server.on("/stop", stopRobot); // Arrête le robot
  server.onNotFound(handleNotFound);
  server.on("/sequence-finie",stopRobot);
  server.begin();
  ...
  attachInterrupt(digitalPinToInterrupt(ENC_G_CH_A), handleLeftEncoder, RISING);
  attachInterrupt(digitalPinToInterrupt(ENC_D_CH_A), handleRightEncoder, RISING);
  if (imu.begin() != 0) {
